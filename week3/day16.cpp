...
//Solution 1 : Using regex library that is the regular expression check
auto i1 = []()
{
...
//solution 2 : conditional approach with strtok
class Solution {
public:
    string validIPAddress(string IP) {
        return isValidIP4(IP)?"IPv4":(isIP6(IP)?"IPv6":"Neither");
    }
    ...
    private:
        bool isValidIP4(string ip)
        {
                ...
                {
                    part_count++;     
                    if(token.size()>3 || token.empty() || token.size()>1 && token[0]='0')
                            return false;
